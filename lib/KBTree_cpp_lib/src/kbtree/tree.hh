...
//
// This version was modified by Michael Sneddon to support certain
// operations needed by trees in the KBase API.
// see http://kbase.us
//
...
class tree {
	...
	public:
      ...
      leaf_iterator   end_leaf(const iterator_base& top) const;
		...
		/// also erases childredn of this node
		template<typename iter> iter erase(iter);
		/// Erase all children of the node pointed to by iterator.
		void     erase_children(const iterator_base&);
		/// Erase just the node pointed to, and reparent its children to point to its parent
		template<typename iter> iter erase_and_reparent_children(iter);
...
template<class T, class tree_node_allocator>
template<class iter>
iter tree<T, tree_node_allocator>::erase_and_reparent_children(iter it) {

	// remember all the details of this current node
	tree_node *cur=it.node;
	tree_node *parent=cur->parent;
	tree_node *prev_sibling=cur->prev_sibling;
	tree_node *next_sibling=cur->next_sibling;
	assert(cur!=NULL); assert(parent!=NULL);

	// handle the first_child case
	if(cur->first_child!=NULL) {
		cur->first_child->parent=parent;
		if(prev_sibling!=NULL) {
			cur->first_child->prev_sibling = prev_sibling;
			prev_sibling->next_sibling = cur->first_child;
		} else {
			parent->first_child=cur->first_child;
		}
	}
	// handle the last_child case
	if(cur->last_child!=NULL) {
		cur->last_child->parent=parent;
		if(next_sibling!=NULL) {
			cur->last_child->next_sibling = next_sibling;
			next_sibling->prev_sibling = cur->last_child;
		} else {
			parent->last_child=cur->last_child;
		}
	}

	// label all child nodes to point to the parent
	for(tree_node *n=cur->first_child; n!=NULL; n=n->next_sibling) {
		n->parent = parent;
	}
